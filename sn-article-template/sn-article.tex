...
\title[Testing utility planning and affordability in Brazil]{Testing Whether Utility Planning Moves Water and Sanitation Affordability A Three Decade Municipal Panel and Coverage Forecast for Brazil}
...
\abstract{Water and sanitation affordability is commonly assessed through the share of household income committed to service tariffs, but whether affordability responds to the planning choices utilities make from one year to the next, namely the level and stability of investment, the source of financing and the control of billing losses, remains largely untested, particularly in a Global South setting marked by wide regional inequality and an unmet universalization deadline. This study tests that premise directly for Brazil. We construct a municipal panel covering 5,549 municipalities and 119,256 municipality year observations from 1995 to 2022, measuring the tariff burden faced by connected customers with an income commitment ratio based on the average residential water and sanitation bill relative to the national minimum wage, a municipal level proxy for affordability rather than a direct household level equity measure. Two way fixed effects models are combined with regional interaction terms and four robustness specifications. None of the four lagged planning variables shows a statistically robust within municipality association with this tariff burden across the full 1995 to 2022 panel, while heterogeneity across municipalities accounts for the large majority of the variation, a pattern that recurs across specifications though it is not uniform across the full period, two variables turn significant when the sample is restricted to 2012 to 2022, and it does not by itself rule out planning effects the available variables or lag structure cannot detect. The burden also varies sharply and monotonically with municipal population size, more than doubling between the smallest and the largest municipalities, a customer facing equity gradient that current planning indicators do not explain. Two out of sample machine learning exercises, one forecasting water coverage and one forecasting the affordability indicator itself, both one year ahead against a chronological holdout, find that the same planning, economic and climate covariates add essentially no accuracy beyond a naive rule that repeats each municipality's last observed value, though for affordability a few economic and investment covariates carry a small but genuinely positive signal that the linear panel specification is not positioned to detect. The coverage affordability trade off that does exist is concentrated in specific regions rather than national, population and economic growth affect access more than price, and water and sanitation follow largely independent trajectories toward the 2033 legal targets, so that projected progress is sensitive to region, service and forecasting method alike. Projecting the tariff burden itself to 2033 with a linear extrapolation and two recursive machine learning models, gradient boosting and random forest, produces three conditional scenarios rather than a validated forecast or a likely range, the linear extrapolation reaches a national average of 4.5 percent, back inside the international affordability range, while random forest reaches 6.0 percent and gradient boosting 7.0 percent, both above it, a directional disagreement reported here to make the uncertainty in any 2033 statement about affordability explicit rather than to resolve it.}
...
\begin{table}[h]
\caption{Planning variable coefficients, dependent variable ICR-SM unless noted, full panel and 2012 to 2022}\label{tab_periodo_2012_2022}
\begin{tabular}{@{}lccc@{}}
\toprule
...
\botrule
\end{tabular}
\footnotetext{$^{**}p<0.01$, $^{***}p<0.001$; coefficients without a star are not significant at the 5 percent level. The state income denominator column uses a state year average wage from the PNADC household survey in place of the national minimum wage.}
\end{table}
...
Set against the aim of this special issue, the account that emerges is one in which the four planning instruments most readily reported by this sector show no robust association with the tariff burden in this panel, while a customer facing equity gradient tied to municipal size and region, demographic pressure that falls on access rather than price, sector specific rather than joint targets, and forecasts honest about their own uncertainty, together point toward a different and more targeted equity agenda than the one the sector currently measures itself against.
